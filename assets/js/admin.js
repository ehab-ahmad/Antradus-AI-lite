(function () {
    ...
    const isInBlockEditor = window.parent !== window;
    const editorWin       = isInBlockEditor ? window.parent : window;
    const editorWp        = (typeof editorWin.wp !== 'undefined') ? editorWin.wp : null;
    ...
    document.getElementById('antradus-generate-btn').addEventListener('click', async function () {
        ...
        if (!keyword && !url) {
            setStatus('Please enter a topic or a source URL.', 'error');
            return;
        }

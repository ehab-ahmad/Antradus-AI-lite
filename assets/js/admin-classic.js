(function () {
    ...
    // ── Image button state (reads TinyMCE directly for pre-existing content) ──
    ...
    function syncImageButtonState() {
        ...
        if (typeof tinymce !== 'undefined') {
            ...
            if (ed && !ed.isHidden()) {
                ...
                var words  = edText.split(/\s+/).filter(Boolean).length;
                if (words >= 50) {
                    btn.disabled = false;
                    if (!generatedArticleText) generatedArticleText = extractImageContext(ed.getContent());
                } else {
                    btn.disabled = true;
                }
                return;
        ...
        if (ta && ta.value.trim()) {
            ...
            if (words >= 50) {
                btn.disabled = false;
                if (!generatedArticleText) generatedArticleText = extractImageContext(ta.value);
            } else {
                btn.disabled = true;
            }
            return;
        ...
        if (words >= 50) {
            ...
            if (!generatedArticleText) {
                ...
                if (ta2) generatedArticleText = extractImageContext(ta2.value);
            }
        } else {
            btn.disabled = true;
        }
    ...
    function startWordCountObserver() {
        ...
        if (wcEl) {
            ...
            new MutationObserver(syncImageButtonState).observe(wcEl, { childList: true, subtree: true, characterData: true });
        } else {
            setTimeout(startWordCountObserver, 500);
        }
    }
    ...
    document.getElementById('antradus-generate-btn').addEventListener('click', async function () {
        ...
        try {
            ...
            if (result.meta_title || result.meta_desc) {
                ...
                if (titleField && result.meta_title) {
                    titleField.value = result.meta_title;
    ...
    document.getElementById('antradus-image-btn').addEventListener('click', async function () {
        if (!generatedArticleText) {
            var ed = typeof tinymce !== 'undefined' ? tinymce.get('content') : null;
            if (ed && !ed.isHidden()) {
                generatedArticleText = ed.getContent({ format: 'text' }).trim().substring(0, 800);
            } else {
                var ta = document.getElementById('content');
                if (ta) generatedArticleText = ta.value.replace(/<[^>]+>/g, ' ').trim().substring(0, 800);
            }
        }
    ...
    // ── Modal (classic editor uses popup) ─────────────────────────────────────
    ...
    function refreshFeaturedImageUi(attachmentId, pid) {
        var wpRef = typeof wp !== 'undefined' ? wp : null;
    ...
    function pasteIntoEditor(html) {
        ...
        var ta = document.getElementById('content');
        if (ta) { ta.value = html; ta.dispatchEvent(new Event('input')); }
    }
    ...
    function detectSeoPlugin() {
        ...
        if (document.getElementById('rank_math_seo') || document.querySelector('.rank-math-metabox') || document.querySelector('[id*="rank_math"][class*="postbox"]')) return 'rankmath';
